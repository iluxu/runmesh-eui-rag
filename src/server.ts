...
const LIVE_MODE = process.env.EUI_LIVE !== "0";
const USE_LOCAL = process.env.EUI_USE_LOCAL === "1";
const SAVE_ON_REFRESH = process.env.EUI_SAVE === "1";
...
const URL_EXCLUDE = (process.env.EUI_URL_EXCLUDE ?? "")
  ...
  .map((entry) => entry.trim())
  .filter(Boolean);
const SEED_URLS = (process.env.EUI_SEED_URLS ?? "")
  .split(",")
  .map((entry) => entry.trim())
  .filter(Boolean);
...
function createTools() {
  ...
  tools.register(
    tool({
      name: "search_docs",
      description: "Search EUI docs for relevant passages.",
      schema: z.object({
        query: z.string(),
...
async function handleAskStructured(req: http.IncomingMessage, res: http.ServerResponse) {
  const body = await parseBody(req);
  const prompt = typeof body.prompt === "string" ? body.prompt : "Summarize EUI banner component.";
  const sessionId = typeof body.sessionId === "string" ? body.sessionId : crypto.randomUUID();
  ...
  try {
    ...
    const promptText = [
      "You are an expert EUI documentation assistant.",
